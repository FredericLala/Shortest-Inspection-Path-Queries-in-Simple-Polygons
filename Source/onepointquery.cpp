...
void OnePointQuery::executeOnePointQuery(QPointF& startingPoint, QPointF& queryPoint, Polygon_2& polygon, const Surface_mesh& mesh)
{
    ...
    std::cout << "END OF QUERY: " << result.log.toStdString() << "\n";
    std::cout << "-------------\n";
    std::cout << "\n";
    ...
    double optimalPathLength = calculatePathLength(optimalPath);

    qint64 elapsedTime = timer.elapsed();
    std::cout << "OnePointQuery completed in " << elapsedTime << " ms \n";
    std::cout << "OnePointQuery optimal path length " << optimalPathLength << "\n";
}
...
double OnePointQuery::calculatePathLength(const QVector<QPointF>& path) {
    double length = 0.0;
