...
QPointF GeneralCase::findLastWaistPoint(const QVector<QPointF>& side1, const QVector<QPointF>& side2) {
    QVector<QPointF> waistPoints;

    for (const QPointF& p1 : side1) {
        for (const QPointF& p2 : side2) {
            if (areEqual(p1, p2)) {
                waistPoints.append(p1);
            }
        }
    }

    return waistPoints.isEmpty() ? QPointF() : waistPoints.last();
}
                                                    ...
                                                    const QVector<QPointF>& funnelSideHelper2)
{
    ...
    // test if hourglass side is degen (in intersection case)
    int hourglassSize = hourglassSide.size();
    if (hourglassSize == 2) {
        if (areEqual(hourglassSide[0], hourglassSide[1])) {
            hourglassSize = 1;
        }
    }

    if (areEqual(hourglassPoint, hourglassSide.first())) {
        Polygon_2 bounds;
        if (hourglassSize > 1) {
            bounds.push_back(Point_2(window.p1().x(), window.p1().y()));
            bounds.push_back(Point_2(window.p2().x(), window.p2().y()));
            bounds.push_back(Point_2(mirrorSide.begin()[1].x(), mirrorSide.begin()[1].y()));
            ...
            QLineF tangentLine = QLineF(funnelPoint, hourglassPoint);
            QLineF extendedLine = extendLine(tangentLine, 0.00001);
            Point_2 testPoint = Point_2(extendedLine.p2().x(), extendedLine.p2().y());
    ...
    if (areEqual(funnelPoint, funnelSide.first()) && areEqual(hourglassPoint, hourglassSide.last()) && hourglassSize > 1) {
...
QVector<QPointF> GeneralCase::tangentBinary(const QVector<QPointF>& funnelSide, const QVector<QPointF>& hourglassSide, const QLineF& window,
    ...
    while (funnelLeft <= funnelRight) {
        ...
        while (hourglassLeft <= hourglassRight) {
            ...
            if (hourglassMid == 0 && funnelMid == funnelVec.size() - 1) {
                hourglassLeft++;
                hourglassMid = hourglassLeft;
            }
...
GeneralCase::FunnelStar GeneralCase::concatenateClosedHourglass(QVector<QPointF>& tangent1, QVector<QPointF>& tangent2, QVector<QPointF>& tangent3, QVector<QPointF>& tangent4, FunnelStruct& funnel, HourglassStruct& hourglass, QPointF& waistPoint) {
    ...
    if (!tangent1.isEmpty()) {
        ...
        QVector<QPointF> secondSide;
        bool start = false;
        for (QPointF point : hourglassSide2) {
            if (areEqual(point, waistPoint)) {
                start = true;
        ...
        funnelStar.funnelStarSide2 = secondSide;
        funnelStar.m3 = waistPoint;
        funnelStar.m4 = waistPoint;
    }
    else if (!tangent2.isEmpty()) {
        QVector<QPointF> secondSide;
        bool start = false;
        for (QPointF point : hourglassSide1) {
            if (areEqual(point, waistPoint)) {
                start = true;
        ...
        funnelStar.funnelStarSide1 = secondSide;
        funnelStar.m1 = waistPoint;
        funnelStar.m2 = waistPoint;
...
GeneralCase::ConcatenatedSideStruct GeneralCase::concatenateOpenHelper(QVector<QPointF>& funnelSide, QVector<QPointF>& hourglassSide, QVector<QPointF>& tangent) {
    ...
    bool start = false;
    for (QPointF point : hourglassSide) {
        if (areEqual(point, tangent.last())) {
            start = true;
            continue;
        }
...
QVector<QPointF> GeneralCase::computeOptimalPathRootInFunnel(QLineF& window1, QLineF& window2, FunnelStar& funnelStar, QVector<QPointF>& pathStartToFunnelRoot) {
    ...
    for (QPointF point: pathStartToFunnelRoot) {
        optimalPath.append(point);
    }

    if (areEqual(pathRootToA2.last(), pathRootToB2.last())) {
        for (QPointF point : pathRootToA2) {
            optimalPath.append(point);
        }
...
QVector<QPointF> GeneralCase::computeOptimalPathRootInHourglass(QLineF& window1, QLineF& window2, FunnelStar& funnelStar, QVector<QPointF>& pathStartToFunnelRoot, QPointF& waistPoint) {
    QVector<QPointF> pathRootToA2;
    QVector<QPointF> pathRootToB2;
    QVector<QPointF> optimalPath;
    QVector<QPointF> funnelStarSide1 = funnelStar.funnelStarSide1;
    ...
    bool start = false;
    for (QPointF point : funnelStarSide1) {
        if (areEqual(point, waistPoint)) {
            start = true;
        }
        if (start) {
            pathRootToA2.append(point);
        }
    }

    start = false;
    for (QPointF point : funnelStarSide2) {
        if (areEqual(point, waistPoint)) {
            start = true;
        }
        if (start) {
            pathRootToB2.append(point);
        }
    }
    ...
    int rootInPath1 = 0;
    int rootInPath2 = 0;
    int waistPointCount1 = 0;
    int waistPointCount2 = 0;
    int occurrences = 0;

    for (const QPointF& p : funnelStarSide1) {
        if (areEqual(waistPoint, p)) {
            waistPointCount1++;
        }
    }

    for (const QPointF& p : funnelStarSide2) {
        if (areEqual(waistPoint, p)) {
            waistPointCount2++;
        }
    }

    if (areEqual(waistPoint, funnelStarSide1.first()) && waistPointCount1 == 1) {
        for (const QPointF& p : funnelStarSide2) {
            if (areEqual(waistPoint, p)) {
                occurrences++;
                if (occurrences == waistPointCount2) { // Break only on the last occurrence
                    break;
                }
            }
            rootInPath2++;
            optimalPath.append(p);
        }
    }
    else if (areEqual(waistPoint, funnelStarSide2.first()) && waistPointCount2 == 1) {
        for (const QPointF& p : funnelStarSide1) {
            if (areEqual(waistPoint, p)) {
                occurrences++;
                if (occurrences == waistPointCount1) { // Break only on the last occurrence
                    break;
                }
            }
            rootInPath1++;
    ...
    OptimalPointStruct c = computeOptimalPoint(pathRootToA2, pathRootToB2, window1, window2);
    int perpendicularPointIndex = c.perpendicularPointIndex + funnelStarRootIndex;
    QPointF optimalPoint = c.optimalPoint;

    if (c.onPathRootToA) {
        for (size_t i = rootInPath1; i <= rootInPath1 + perpendicularPointIndex; ++i) {
            optimalPath.append(funnelStarSide1[i]);
        }

        if (rootInPath1 + perpendicularPointIndex == funnelStarSide1.size() - 1) { // if c == a
            return optimalPath;
        }
    }
    else {
        for (size_t i = rootInPath2; i <= rootInPath2 + perpendicularPointIndex; ++i) {
            optimalPath.append(funnelStarSide2[i]);
        }

        if (rootInPath2 + perpendicularPointIndex == funnelStarSide2.size() - 1) { // if c == b
            return optimalPath;
        }
    }


    optimalPath.append(optimalPoint);
    return optimalPath;
}
...
GeneralCase::GeneralCaseResult GeneralCase::executeGeneralCase(QPointF& startingPoint, QLineF& window1, QLineF& window2, Polygon_2& polygon, Surface_mesh& mesh)
{
    ...
    FunnelStar funnelStar;
    QPointF waistPoint = findLastWaistPoint(hourglass.hourglassSide1, hourglass.hourglassSide2);
    if (!hourglass.isOpen) {
        funnelStar = concatenateClosedHourglass(tangent1, tangent2, tangent3, tangent4, funnel, hourglass, waistPoint);
    }
    else {
    ...
    if (funnelStar.isRootInFunnel) {
        ...
        optimalPath = computeOptimalPathRootInHourglass(window1, window2, funnelStar, pathStartToFunnelRoot, waistPoint);
    }
